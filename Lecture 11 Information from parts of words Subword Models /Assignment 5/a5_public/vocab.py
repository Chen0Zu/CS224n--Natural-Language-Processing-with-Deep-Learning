...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            sent2idx = []
            for word in sent:
                word2idx = [self.start_of_word]
                for char in word:
                    word2idx.append(self.char2id[char])
                word2idx.append(self.end_of_word)
                sent2idx.append(word2idx)
            word_ids.append(sent2idx)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_var = torch.LongTensor(sents_padded).permute(1, 0, 2)
        return sents_var
        ### END YOUR CODE
